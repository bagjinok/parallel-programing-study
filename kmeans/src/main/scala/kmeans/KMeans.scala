...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    var grouped = points.groupBy(point =>  findClosest(point, means))

    for (mean <- means) {
      if (grouped.get(mean) eq None) {
        grouped = grouped.updated(mean, IndexedSeq())
      }
    }
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    ...
    for (mean <- means) {
      if (grouped.get(mean) eq None) {
        grouped = grouped.updated(mean, IndexedSeq().par)
      }
    }
  ...
  def update(classified: Map[Point, Seq[Point]], oldMeans: Seq[Point]): Seq[Point] = {
    for (oldMean <- oldMeans) yield {
      val clusteredPoints = classified.get(oldMean)
      if (clusteredPoints ne None) {
        findAverage(oldMean, clusteredPoints.get)
      } else {
        oldMean
      }
    }
  ...
  def converged(eta: Double, oldMeans: Seq[Point], newMeans: Seq[Point]): Boolean = {
    val converged = for ((oldMean, newMean) <- oldMeans zip newMeans)
      yield oldMean.squareDistance(newMean) <= eta

    converged.reduce((c1, c2) => c1 && c2)
  }
